...
### 2026-06-16 — live persona verification of the 06-13 handover (J10 fixed, map fix confirmed)
- **The 06-13 close predicted exactly this and it paid off:** the device-handover note said "live-verify chat links next session." Doing so immediately surfaced a P1 the local QA couldn't: every AI-chat `/help/*.png` screenshot was a broken image on production. **Root cause: `.gitignore` line 41 is a blanket `*.png`** — when the 06-13 session "added 5 screenshots in public/help/", `git add` silently skipped them, and local dev served them fine from disk so nobody noticed. On prod they fell through the SPA catch-all rewrite (`/((?!api/).*) → /index.html`), returning index.html as 200/text/html. **Fix:** `!public/help/*.png` negation + committed the assets (6113699); verified live. *Pattern (new, log it): a feature that references a static asset path is a deploy contract — verify the asset is actually committed AND served from prod (curl the path, assert content-type), not just present on local disk. This is the asset-layer twin of the "prompt references a doc section that doesn't exist" bug class.*
- **Live-UI verification remains the only honest gate.** Bundle markers, code reading, and local dev ALL looked fine; only fetching the prod URL and rendering the `<img>` in a real browser exposed it. Re-confirms §3.3.
- **Verification honesty — what I did NOT prove:** Team Add Member reached GoTrue but never created a user (GoTrue rejects `+`/`.` aliases as "invalid"; then I hit Supabase's hourly email rate limit — and consumed Mark's invite quota for ~1h as a side effect). Remove is code-verified only; I declined to delete a live persona account to test it. Both Team flows still need one clean live round-trip before "done" — recorded as PARTIAL, not green-washed.
- **Findings handed forward:** (1) GoTrue "Email address … is invalid" for `+`/`.` aliases is a confusing UX message for valid-format addresses — consider client-side guidance or a clearer error. (2) `api/remove-user.js` deletes the profile row before the auth user; an auth-delete failure leaves the orphan it was built to prevent — reorder (auth first, then profile) or make it transactional. (3) A stray "(No name)" investor profile (id 78b5040d…) sits in the team list — likely an orphan to clean up.
- **EMAIL INFRA — P0 for UAT and likely a live booking bug (found 2026-06-16 while inviting Andrew).** Supabase Auth uses the **built-in email service capped at 2 emails/hour** with **no custom SMTP** (`smtp_host:null`); that's why bulk invites fail. Worse: with the Resend key in `.env.local` (a restricted send-only key), **neither `binned-it.com.au` nor `binnedit.com.au` is a verified Resend domain** (both return 403 "domain is not verified" on a test send). Since `api/book-confirm.js` hardcodes the sender `noreply@binned-it.com.au` (the unverified domain), **booking-confirmation emails are probably silently failing in production** — verify against a real booking. Fix path for UAT: verify a company domain in Resend (DNS SPF/DKIM — Mark's action), then point Supabase Auth SMTP at Resend (smtp.resend.com:465, user `resend`, pass=RESEND_API_KEY) and raise `rate_limit_email_sent`. Caveat: `.env.local` may hold the *dev* Resend key; confirm whether prod Vercel env has a different key with a verified domain. *Decision (Mark, 2026-06-16): send Andrew now via the built-in 2/hr service (auto-retry pending the window), verify Resend domain as a follow-up.*
- **Standing decision logged: Andrew is being onboarded as `owner`** (not investor) at andrew@binned-it.com.au for full human UAT — Mark confirmed twice, explicitly overriding the investor-sandbox model for Andrew. This means Andrew's J9 low-privilege assumptions NO LONGER hold for this account; the investor-sandbox security tests still apply to any *investor/viewer*-role account, just not to Andrew personally now.

### 2026-06-13 — map stacking fix, Team add/remove, AI chat deep links (device handover)
- **Shipped + committed earlier this session:** LiveMapPanel stacking-context isolation (Leaflet's z-1000 controls were bleeding over the side menu and chat panel — fixed by containing the map in its own stacking context, not by escalating z-indexes elsewhere); Team page Add Member + Remove (owner-only; invite with name/email/role; `api/remove-user.js` deletes auth user + profile together so no orphan rows).
- **Shipped this commit:** AI chat deep links — the assistant now emits `[Page](/route)` markdown for every place it points the user at, plus `![…](/help/*.png)` screenshots; ChatPanel renders internal links as SPA-navigating buttons (closes the panel on mobile) and images inline. **Contract bug caught at session close:** the prompt told the model to use the "Help screenshots" list in the manual, but no such list existed in `api/lib/manual.js` — the model would have invented `/help/` paths or never shown images. List added (5 screenshots in `public/help/`). *Pattern (recurring): a prompt that references a document section is a contract — verify the section exists, same class as the SSE `{text}` and prop-threading bugs.*
- **Verification honesty:** chat deep links were QA'd locally (qa-chat-*.png) but go live only with this push — J10's link/screenshot behaviour needs a live persona run (Mark, both viewports) next session before it counts as proven. Board statuses unchanged.
- **Session context:** Mark is switching devices; this close exists so the next device starts with zero context loss. Next session: live-verify chat links, then resume triage.md Wave 3 (data wiring).
