...
// ─────────────────────────────────────────────────────────────────────────────
// Shared booking form (Sprint 16 #37).
//
// Single source of truth for the multi-step public booking flow that powers
// both `/book` (BookingPage — default Binned-IT tenant) and `/embed/<slug>`
// (EmbedBookingPage — white-label tenant widget).
//
// Both flows:
//   - render the same 4 steps (Bin Size → Customer Details → Delivery → Review),
//   - share validation, palette/context, success screen and trust badges,
//   - INSERT into the public.bookings table with the tenant's `tenant_id`
//     (RLS migration 013 allows anon insert),
//   - POST to /api/book-confirm so the Sprint 13 SMS + email pipeline still
//     fires for both routes (do not change that contract).
//
// Page-specific concerns (SEO/JSON-LD on /book, tenant header/contact on
// /embed/<slug>) stay in their respective page wrappers — this component
// only owns the form itself.
// ─────────────────────────────────────────────────────────────────────────────

// ─── Palette context — themed per tenant primary_color ───────────────────────
...
export function makePalette(primaryColor) {
...
function FormSelect({ value, onChange, children, hasError }) {
  const p = useC();
  const [focused, setFocused] = useState(false);
  return (
    <select
    ...
    >
      {loading && (
        <span style={{
          ...
          borderRadius: '50%', display: 'inline-block', animation: 'booking-form-spin 0.7s linear infinite',
        }} />
      )}
      {children}
    </button>
...
function StepBinSize({ data, setData, errors, bins }) {
  const p = useC();
  const sizeMap = { 0: 28, 1: 36, 2: 44, 3: 52 };
  return (
    <section aria-labelledby="booking-step1-heading">
      <h2 id="booking-step1-heading" style={{ fontSize: 22, fontWeight: 700, color: p.text, marginBottom: 4 }}>
...
function StepCustomerDetails({ data, setData, errors }) {
  ...
  return (
    <section aria-labelledby="booking-step2-heading">
      <h2 id="booking-step2-heading" style={{ fontSize: 22, fontWeight: 700, color: p.text, marginBottom: 4 }}>
        Your Details
      </h2>
      <p style={{ fontSize: 14, color: p.textMuted, marginBottom: 24 }}>
...
function StepDeliveryDetails({ data, setData, errors }) {
  ...
  return (
    <section aria-labelledby="booking-step3-heading">
      <h2 id="booking-step3-heading" style={{ fontSize: 22, fontWeight: 700, color: p.text, marginBottom: 4 }}>
        Delivery Details
      </h2>
      <p style={{ fontSize: 14, color: p.textMuted, marginBottom: 24 }}>
...
function StepReview({ data, termsChecked, setTermsChecked, errors, tenant }) {
  ...
  return (
    <section aria-labelledby="booking-step4-heading">
      <h2 id="booking-step4-heading" style={{ fontSize: 22, fontWeight: 700, color: p.text, marginBottom: 4 }}>
        Review Your Booking
      </h2>
      <p style={{ fontSize: 14, color: p.textMuted, marginBottom: 24 }}>
...
// ─── Header / Hero / Footer (rendered when wrapper opts in) ──────────────────

function FormHeader({ tenant }) {
  const p = useC();
  return (
...
/**
 * Shared multi-step booking form.
 *
 * Props:
 *   tenant   — { id, company_name, primary_color, phone, email, suburb,
 *                logo_url, terms_url } (from `tenants` table; id used for
 *                the bookings.tenant_id FK and may be null for fallback).
 *   bins     — array of bin sizes (from `tenant_bin_sizes` table) with shape:
 *              { id, size_label, volume_m3, description, price, popular }
 *   showHeader — (default true) render the tenant header / hero strip.
 *   showFooter — (default true) render the tenant footer.
 *   showTrustBadges — (default true) render the trust-badges row.
 *   minHeight  — optional minHeight override for the outer container.
 */
export default function BookingForm({
  ...
  const goNext = useCallback(() => {
    ...
    (async () => {
      try {
        ...
        // Sprint 13 #21 — Twilio SMS + email pipeline. Fire-and-forget; do not
        // block the success screen on the network call.
        fetch('/api/book-confirm', {
          ...
          if (typeof console !== 'undefined' && console.warn) {
            console.warn('[book-confirm] non-fatal:', err?.message || err);
          }
        });
  ...
  if (submitted) {
    return (
      <>
        <style>{`@keyframes booking-form-spin { to { transform: rotate(360deg); } }`}</style>
        <C.Provider value={palette}>
          <SuccessScreen bookingRef={bookingRef} data={data} tenant={tenant} />
        </C.Provider>
      </>
    );
  }
  ...
  return (
    <>
      <style>{`@keyframes booking-form-spin { to { transform: rotate(360deg); } }`}</style>
      <C.Provider value={palette}>
        <div style={{ background: palette.bg, minHeight, fontFamily: '"DM Sans", system-ui, sans-serif', color: palette.text }}>

          {showHeader && <FormHeader tenant={tenant} />}

          {showHeader && (
            <div style={{ background: palette.primary, padding: '10px 24px', textAlign: 'center' }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: palette.primaryText, margin: 0 }}>
          ...
          <main style={{ maxWidth: 700, margin: '32px auto', padding: '0 16px 48px' }}>
            <div style={{ background: palette.card, borderRadius: 16, boxShadow: palette.shadowLg, padding: '32px 28px' }}>
              ...
              {submitError && (
                <div style={{ background: palette.redBg, border: `1px solid ${palette.red}`, borderRadius: 8, padding: '12px 14px', marginTop: 16, color: palette.red, fontSize: 14 }}>
                  {submitError}
                </div>
              )}
            ...
            {showTrustBadges && (
              <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 24, flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: palette.textMuted }}>
          ...
          {showFooter && <FormFooter tenant={tenant} />}
        </div>
      </C.Provider>
    </>
  );
...
// ─── Fallback bins for offline / no-Supabase scenarios ───────────────────────
// Mirrors the seeded `tenant_bin_sizes` rows for the `binned-it` default tenant
// (migration 012). Used by both wrapper pages when Supabase is unreachable.
export const FALLBACK_BINS = [
